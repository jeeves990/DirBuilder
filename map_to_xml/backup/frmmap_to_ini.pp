...
begin
  ...
  if aCol = 0 then
  begin
    fldName := FmMap_to_INI.Grid.Cells[0, aRow];
    FmMap_to_INI.StatusBar.SimpleText := Format('Cell[%d, %d]', [aCol, aRow]);
	end;
end;

{ TFmMap_to_INI }

procedure TFmMap_to_INI.FormCreate(Sender : TObject);
begin
...
procedure TFmMap_to_INI.FormShow(Sender : TObject);
begin
  FFormCreateFlag := False;
end;

function TFmMap_to_INI.getComboBoxWidth : Integer;
var
  idx, sWidth : Integer;
begin
  Result := 0;
...
procedure TFmMap_to_INI.SetReportTypes(AValue : TStringList);
var
  i : Integer;
begin
...
procedure TFmMap_to_INI.GridSelectEditor(Sender : TObject; aCol,
...
procedure TFmMap_to_INI.GridShowHint(Sender : TObject; HintInfo : PHintInfo);
var
  pt : TPoint;
  rect : TRect;
  idx : Integer;
...
procedure TFmMap_to_INI.AddDBColumnNames;
var
...
procedure {TFmMap_to_INI.}DisplayHint(Sender : TObject; HintInfo : PHintInfo);
var
  pt : TPoint;
begin
  {  1. which cell are we over.  }
...
procedure TFmMap_to_INI.FormClose(Sender : TObject; var CloseAction : TCloseAction);
begin
  //FDmod.Free;
  FFldList.Free;
...
procedure TFmMap_to_INI.ActionAddColumnsFromBooksExecute(Sender : TObject);
begin
  AddDBColumnNames;
end;

procedure TFmMap_to_INI.ActionAddNewColumnExecute(Sender : TObject);
var
  rowIdx, colIdx : Integer;
begin
...
procedure TFmMap_to_INI.ActionGetReportTypeExecute(Sender : TObject);
begin
  Get_report_type('another report');
end;

function TFmMap_to_INI.Get_report_type(rpt_type : String) : String;
var
...
procedure TFmMap_to_INI.ActionReadColumnFromINIExecute(Sender : TObject);
begin
//
end;

procedure TFmMap_to_INI.ActionResizeGridColumnsExecute(Sender : TObject);
var
...
procedure TFmMap_to_INI.ActionWriteCol2INIExecute(Sender : TObject);
const
  fmt_str = '%s=%s';
var
  aCol, rowDx : Integer;
...
procedure TFmMap_to_INI.cboxReportsCloseUp(Sender : TObject);
var
  cbox : TComboBox;
begin
